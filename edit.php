...
if (isset($_POST['update'])) {
	...
	$data = mysqli_query($conn, "SELECT foto FROM users WHERE id='$id'");
	...
	if (empty($name) || empty($mobile) || empty($email)){
		...
		if (!empty($filename)) {
			...
			$result = mysqli_query($conn, "UPDATE users SET name='$name',mobile='$mobile',email='$email',foto='$filename' WHERE id=$id");
		}

		// MEMASUKAN DATA YANG DI UPDATE KECUALI GAMBAR
		$result = mysqli_query($conn, "UPDATE users SET name='$name',mobile='$mobile',email='$email' WHERE id=$id");
